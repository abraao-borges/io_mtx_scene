...
def import_nodearray(gamemode):
    ...
    for node in NodeArray:
        ...
        if "Name" in node and "Class" in node:
            ...
            # Create level lights - these are point lights and not empties
            if node["Class"] == "LevelLight":
                light_data = bpy.data.lights.new(name="Lamp_" + node["Name"], type='POINT')
                if "Brightness" in node:
                    if node["Brightness"] != 0:
                        light_data.energy = float(node["Brightness"])
                    else:
                        light_data.energy = 100.0
                        
                if "InnerRadius" in node:
                    light_data.thug_light_props.light_radius[0] = float(node["InnerRadius"])
                if "OuterRadius" in node:
                    light_data.thug_light_props.light_radius[1] = float(node["OuterRadius"])
                if "Color" in node:
                    light_data.color[0] = float(int(node["Color"][0]) / 255)
                    light_data.color[1] = float(int(node["Color"][1]) / 255)
                    light_data.color[2] = float(int(node["Color"][2]) / 255)
                if "ExcludeSkater" in node:
                    light_data.thug_light_props.light_excludeskater = True
                if "ExcludeLevel" in node:
                    light_data.thug_light_props.light_excludelevel = True
                    
                ob = bpy.data.objects.new(name=node["Name"], object_data=light_data)
