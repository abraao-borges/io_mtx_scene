...
def preset_place_node(node_type, position):
    ...
    if node_type == 'RESTART':
        ...
        scene.view_layer.objects.active = curveOB
        curveOB.select = True
...
def sanitize_menu_name(raw_name):
...
def preset_place_mesh(dictionary_name, piece_name, position):
    ...
    #scene.collection.objects.link(new_piece)
    scene.view_layer.objects.active = new_piece
...
mesh_submenus = []
mesh_categories = {}
registered_preset_ids = set()


def unique_dynamic_bl_idname(category, *parts):
    name_parts = []
    for part in parts:
        if part is None:
            continue
    ...
    clean_name = '_'.join(name_parts) if name_parts else 'dynamic'
    candidate = f"{category}_{clean_name}"
    suffix = 1
    while candidate in registered_preset_ids:
        candidate = f"{category}_{clean_name}_{suffix}"
        suffix += 1
    registered_preset_ids.add(candidate)
    return candidate


def addPresetNodes():
    for node in preset_node_list:
        clean_name = sanitize_menu_name(node['name']).lower()
        op_name = unique_dynamic_bl_idname('object', f"add_custom_{clean_name}")
        nc = type('DynOp_' + sanitize_menu_name(node['name']),
                    (AddTHUGNode, ),
                    {'bl_idname': op_name,
                    'bl_label': node['title'],
...
def addPresetMesh():
    for template in preset_template_list:
        if not template["name"] in preset_mesh:
            ...
            submenu_id = sanitize_menu_name(template["name"])
            new_submenu = type('DynMenu_' + template["name"],
                        (THUGMeshSubMenu, ),
                        {'bl_idname': unique_dynamic_bl_idname(THUGMeshSubMenu.bl_idname, submenu_id),
        ...
        for cat_name, category in template["list"].items():
            ...
            if not cat_name in preset_mesh:
                ...
                submenu_id = sanitize_menu_name(template["name"]) + '_' + sanitize_menu_name(cat_name)
                new_submenu = type('DynMenu_' + template["name"] + '_' + cat_name,
                        (THUGMeshSubSubMenu, ),
                        {'bl_idname': unique_dynamic_bl_idname(THUGMeshSubSubMenu.bl_idname, submenu_id),
                        'bl_label': cat_name,
                        'template_name': template["name"],
            ...
            for ob in category:
                ...
                if "single" in ob:
                    ...
                    unique_id = unique_dynamic_bl_idname('object', f"add_custom_{template['name']}_{sanitize_menu_name(cat_name)}_{sanitize_menu_name(ob['single'])}")
