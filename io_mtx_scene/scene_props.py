...
def sanitize_scene_prop_value(prop_val):
    if prop_val is None:
        return 'None'
    if hasattr(prop_val, 'default'):
        prop_val = prop_val.default
    elif isinstance(prop_val, tuple) or "PropertyDeferred" in type(prop_val).__name__:
        prop_val = 'None'
    ...
    if isinstance(prop_val, (int, float, bool)):
        return str(prop_val)
    if hasattr(prop_val, '__str__') and not hasattr(prop_val, 'keys'):
        return str(prop_val)
    return 'None'
...
def thug_empty_update(self, context):
    if context.object.type != "EMPTY":
        return
    ob = context.object
    for mdl_ob in list(ob.children):
        if mdl_ob.name.endswith('_MDL'):
            if mdl_ob.name in context.collection.objects:
                context.collection.objects.unlink(mdl_ob)
...
def update_node_collection(*args):
    ...
    if wm is None or not hasattr(wm, "thug_all_nodes"):
        return

    all_nodes = getattr(wm, "thug_all_nodes", None)
    if all_nodes is None:
        return

    paths = getattr(all_nodes, "paths", None)
    if hasattr(all_nodes, "paths") and paths is not None and type(paths).__name__ != "_PropertyDeferred":
        try:
            paths.clear()
        except AttributeError:
            pass
    restarts = getattr(all_nodes, "restarts", None)
    if hasattr(all_nodes, "restarts") and restarts is not None and type(restarts).__name__ != "_PropertyDeferred":
        try:
            restarts.clear()
        except AttributeError:
            pass
    meshes = getattr(all_nodes, "meshes", None)
    if hasattr(all_nodes, "meshes") and meshes is not None and type(meshes).__name__ != "_PropertyDeferred":
        try:
            meshes.clear()
        except AttributeError:
            pass
    scripts = getattr(all_nodes, "scripts", None)
    if hasattr(all_nodes, "scripts") and scripts is not None and type(scripts).__name__ != "_PropertyDeferred":
        try:
            scripts.clear()
        except AttributeError:
            pass
    
    if type(getattr(all_nodes, "meshes", None)).__name__ == "_PropertyDeferred":
        return

    for ob in bpy.data.objects:
        if ob.type == 'MESH' and ( ob.thug_export_collision or ob.thug_export_scene ) and hasattr(all_nodes, "meshes"):
            entry = all_nodes.meshes.add()
            entry.name = ob.name
        elif ob.type == 'EMPTY' and ob.thug_empty_props.empty_type == 'Restart' and hasattr(all_nodes, "restarts"):
            entry = all_nodes.restarts.add()
            entry.name = ob.name
        elif ob.type == 'CURVE' and ob.thug_path_type in [ 'Rail', 'Ladder', 'Waypoint' ] and hasattr(all_nodes, "paths"):
            entry = all_nodes.paths.add()
            entry.name = ob.name
            
    for tx in bpy.data.texts:
        if tx.name.startswith('script_') and hasattr(all_nodes, "scripts"):
            entry = all_nodes.scripts.add()
...
def maybe_upgrade_scene(*args):
    ...
    for mat in bpy.data.materials:
        if hasattr(mat, 'thug_material_props') and hasattr(mat.thug_material_props, 'ugplus_shader') \
            ...
            for passname in newmat_passes:
                thispass = getattr(mat.thug_material_props, passname, None)
                if thispass and hasattr(thispass, "tex_image_name") and type(thispass).__name__ != "_PropertyDeferred":
                    try:
                        tex_name = getattr(thispass, "tex_image_name", None)
                        if tex_name:
                            thisimage = bpy.data.images.get(tex_name)
                            if thisimage:
    ...
    #print("Updating node collections...")
    scene = context.scene
    if 'io_thps_scene_version' not in scene or scene['io_thps_scene_version'] == None:
        print("This blend file was built with the asdf plugin, or a pre-release version of io_thps_scene. Needs to be updated!")
        should_upgrade = True
    elif scene['io_thps_scene_version'] != ADDON_VERSION:
    ...
    if should_upgrade:
        ...
        for ob in bpy.data.objects:
            if ob.type in ['MESH', 'EMPTY', 'CURVE'] and ob.thug_triggerscript_props.triggerscript_type:
                ...
                target = getattr(ob_ts, "target_node", None)
                target_value = sanitize_scene_prop_value(target)
                if target_value and target_value != 'None':
                    target_str = str(target_value)
                    if target_str and bpy.data.objects.get(target_str):
                        # Target node (for Teleport/Killskater) should always be param1 on the new template(s)
                        ob.thug_triggerscript_props.param1_string = get_clean_name(bpy.data.objects.get(target_str))
    ...
    scene['io_thps_scene_version'] = ADDON_VERSION
    
    def draw(self, context):
        self.layout.label(text="This scene was built on an older version of the THPS Blender plugin (io_thug_tools). Your scene has been auto-converted to be compatible with this version of the plugin.")
        self.layout.label(text="However, it may no longer be BACKWARD compatible with the previous plugin. If this is not desired, please make a backup copy before saving the scene.")
        if len(fix_objects) > 0:
            self.layout.label(text="-------------------------------------------")
            self.layout.label(text="Some nodes were unable to be converted automatically, such as restarts/CTF Flags. Please review the following objects, as they may need to be re-configured:")
...
def thug_light_update(self, context):
    if context.object.type == "LIGHT" and context.object.data.type == "POINT":
...
CLASSES = (
    THUGEmptyProps,
    THUGObjectTriggerScriptProps,
    THUGProximNodeProps,
    THUGEmitterProps,
    THUGCubemapProps,
    THUGGenericNodeProps,
    THUGGameObjectProps,
    THUGBouncyProps,
    THUGNodeListProps,
    THUGLevelObjectProps,
    THUGWaypointProps,
    THUGPathNodeProps,
    THUGPathNodeUIProps,
    THUGRestartProps,
    ...
    THUGLightProps,
    THUGParticleProps,
    THUGLevelExportProps,
    THUGLevelProps,
    THUGImageProps,
    THUGAnimatedTextureKeyframe,
    THUGAnimatedTexture,
    THUGUVWibbles,
    UGPlusMaterialSlotProps,
    THUGMaterialProps,
    THUGMaterialPassProps,
)
...
def register_props():
    ...
    global draw_handle
    draw_handle = bpy.types.SpaceView3D.draw_handler_add(ui_draw.draw_stuff, (), 'WINDOW', 'POST_VIEW')
    # bpy.app.handlers.scene_update_pre.append(ui_draw.draw_stuff_pre_update)
    bpy.app.handlers.depsgraph_update_post.append(ui_draw.draw_stuff_post_update)
    ...
    bpy.app.handlers.load_pre.append(ui_draw.draw_stuff_pre_load_cleanup)
...
def unregister_props():
    ...
    """
    if hasattr(ui_draw, 'draw_stuff_pre_update') and ui_draw.draw_stuff_pre_update in bpy.app.handlers.scene_update_pre:
        bpy.app.handlers.scene_update_pre.remove(ui_draw.draw_stuff_pre_update)
    """
    ...
    if ui_draw.draw_stuff_post_update in bpy.app.handlers.depsgraph_update_post:
        bpy.app.handlers.depsgraph_update_post.remove(ui_draw.draw_stuff_post_update)
    ...
    if ui_draw.draw_stuff_pre_load_cleanup in bpy.app.handlers.load_pre:
        bpy.app.handlers.load_pre.remove(ui_draw.draw_stuff_pre_load_cleanup)
...
#----------------------------------------------------------------------------------
def unregister():
    if hasattr(bpy.types.Object, 'thug_levelobj_props'):
        del bpy.types.Object.thug_levelobj_props
    if hasattr(bpy.types.Object, 'thug_triggerscript_props'):
        del bpy.types.Object.thug_triggerscript_props
    if hasattr(bpy.types.Object, 'thug_empty_props'):
        del bpy.types.Object.thug_empty_props
    if hasattr(bpy.types.Object, 'thug_cubemap_props'):
        del bpy.types.Object.thug_cubemap_props
    if hasattr(bpy.types.Object, 'thug_proxim_props'):
        del bpy.types.Object.thug_proxim_props
    if hasattr(bpy.types.Object, 'thug_emitter_props'):
        del bpy.types.Object.thug_emitter_props
    if hasattr(bpy.types.Object, 'thug_generic_props'):
        del bpy.types.Object.thug_generic_props
    if hasattr(bpy.types.Object, 'thug_restart_props'):
        del bpy.types.Object.thug_restart_props
    if hasattr(bpy.types.Object, 'thug_go_props'):
        del bpy.types.Object.thug_go_props
    if hasattr(bpy.types.Object, 'thug_ped_props'):
        del bpy.types.Object.thug_ped_props
    if hasattr(bpy.types.Object, 'thug_veh_props'):
        del bpy.types.Object.thug_veh_props
    if hasattr(bpy.types.Object, 'thug_particle_props'):
        del bpy.types.Object.thug_particle_props
    if hasattr(bpy.types.Object, 'thug_waypoint_props'):
        del bpy.types.Object.thug_waypoint_props
    if hasattr(bpy.types.Object, 'thug_pathnode_props'):
        del bpy.types.Object.thug_pathnode_props
    if hasattr(bpy.types.Curve, 'thug_pathnode_triggers'):
        del bpy.types.Curve.thug_pathnode_triggers
    if hasattr(bpy.types.Light, 'thug_light_props'):
        del bpy.types.Light.thug_light_props
    if hasattr(bpy.types.Image, 'thug_image_props'):
        del bpy.types.Image.thug_image_props
    if hasattr(bpy.types.Material, 'thug_material_props'):
        del bpy.types.Material.thug_material_props
    if hasattr(bpy.types.Texture, 'thug_material_pass_props'):
        del bpy.types.Texture.thug_material_pass_props
    if hasattr(bpy.types.WindowManager, 'thug_all_nodes'):
        del bpy.types.WindowManager.thug_all_nodes
    if hasattr(bpy.types.WindowManager, 'thug_pathnode_props'):
        del bpy.types.WindowManager.thug_pathnode_props
    if hasattr(bpy.types.Scene, 'thug_level_props'):
        del bpy.types.Scene.thug_level_props
