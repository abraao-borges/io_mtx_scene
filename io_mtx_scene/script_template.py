...
def init_templates():
    ...
    package_name = __package__.split('.')[0] if __package__ else ADDON_NAME
    addon = bpy.context.preferences.addons.get(package_name) or bpy.context.preferences.addons.get(ADDON_NAME)
    addon_prefs = addon.preferences if addon else None
    if addon_prefs is None:
        bpy.app.timers.register(show_welcome_popup, first_interval=0.1)
        return
    ...
    template_files = glob.glob(base_files_dir + r"scripts\*.ini")
